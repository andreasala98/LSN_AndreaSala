...
Once compiled, QMC1D is invoked with the command: "./qmc1d". It will read the settings in the file "input.dat".
*/
#include <iostream>
#include <fstream>
#include <cmath>
...
{
  double u=0.8064,s=0.6175;
  return -1./(s*s) * variationalWaveFunction(x) + 1./(s*s*s*s)* ((x-u) * (x-u) * exp(-pow(x-u,2)/(2*s*s)) + (x+u)*(x+u) * exp(-pow(x+u,2)/(2*s*s)));
	//return 0;
}
