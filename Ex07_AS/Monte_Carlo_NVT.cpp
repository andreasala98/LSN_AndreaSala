...
void Averages(int iblk) //Print results for current block
{
   ...
   const int wd=12;
    ...
    Epot.open(phase_type+"_output.epot.0",ios::app);
    Pres.open(phase_type+"_output.pres.0",ios::app);
...
//g(r)
  ...
  if(iblk==nblk){ //Stampo solo l'ultimo blocco in G_ave
...
void InstantPrint(void)
{
    ofstream PrintOut;
    double un, pr;
    
    PrintOut.open(phase_type+"_InstantValues.out",ios::app);
    
    un = walker[iv]/npart + vtail;
    pr = rho * temp + (walker[iw] + ptail*npart)/vol;
